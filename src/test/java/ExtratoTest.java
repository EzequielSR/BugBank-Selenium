...
public class ExtratoTest {
    ...
    private void verificarTransferenciaNoExtrato(double saldoEsperado) {
        ...
        double saldoAtual = Double.parseDouble(saldoTexto);

        Assertions.assertEquals(saldoEsperado, saldoAtual, 0.01, "O saldo disponível não está correto!");
    }
    ...
    public void testRealizarTransferenciaComSucesso() {
        ...
        String descricao = "Transferência de teste";

        realizarTransferencia(contaDestino, digitoDestino, valorTransferencia, descricao);

        driver.findElement(By.xpath("//*[@id=\"btnBack\"]")).click();

        clicarExtrato();

        verificarTransferenciaNoExtrato(900.0);
    }
}